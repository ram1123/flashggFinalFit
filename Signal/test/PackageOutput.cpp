  ...
  RooRealVar *intLumi = new RooRealVar("IntLumi","IntLumi",lumi_*1000,0.,10.e5);
    ...
    for (int i=0 ; i < nFiles; i++) {
      ...
      if (i !=0) {
        ...
        vTmpWS = inWS->getSpecificWorkspace(i);
        tmpWS->import(vTmpWS->allVars(),RecycleConflictNodes());
        tmpWS->import(vTmpWS->allFunctions(),RecycleConflictNodes());
        tmpWS->import(vTmpWS->allPdfs(),RecycleConflictNodes());
